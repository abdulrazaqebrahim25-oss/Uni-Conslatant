...
# Each semester's end date within a calendar year
SEMESTER_WINDOWS = {
    "spring": (1,  5,  31),   # Jan–May
    "summer": (6,  8,  31),   # Jun–Aug
    "fall":   (9,  12, 31),   # Sep–Dec
}
...
def get_elapsed_semesters(entry_semester, entry_year, preferred_per_year=3):
    """
    Returns a list of dicts for every semester that has fully ended
    since the student's entry point.

    preferred_per_year=2  →  spring + fall only  (no summer)
    preferred_per_year=3  →  all three semesters
    """
    today = date.today()
    ...
    for year in range(entry_year, today.year + 1):
        for sem in SEMESTER_ORDER:
            ...
            # Don't count anything before the entry point
            if not started:
            ...
            # Only include semesters that have fully finished
            if end_date < today:
                elapsed.append({
                    "semester": sem,
...
def student_completed_courses_view(request):
    """
    Displays the student's completed courses with semester progress info,
    and lets them update which courses they have finished.
    """
    student = request.user.student_profile
    ...
    # ── POST: student saves an updated list ───────────────────────────────────
    if request.method == "POST":
        selected_ids = request.POST.getlist("completed_courses")

        # Guard: only accept course IDs that actually belong to the major
...
def advisor_dashboard_view(request):
    ...
    return render(request, "dashboard/advisor_dashboard.html", {"advisor": advisor})


@login_required
def advisor_profile_view(request):
    advisor = request.user.advisor_profile
    return render(request, "dashboard/advisor_profile.html", {"advisor": advisor})
...
def edit_advisor_profile_view(request):
    ...
    return render(request, "dashboard/edit_advisor_profile.html", {"form": form})
...
def advisor_students_view(request):
    ...
    return render(request, "dashboard/advisor_students.html", {"students": students})


@login_required
def advisor_student_detail(request, student_id):
    advisor = request.user.advisor_profile
    student = get_object_or_404(StudentProfile, id=student_id, advisor=advisor)
